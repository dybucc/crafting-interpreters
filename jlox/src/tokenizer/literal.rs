...
impl Display for Lit {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { self.repr.fmt(f) }
}

impl From<&[u8]> for Lit {
    fn from(value: &[u8]) -> Self { Self { repr: LitRepr::Str(Str::from(value)) } }
}
...
impl Default for LitRepr {
    fn default() -> Self { Self::Str(Str::default()) }
}
...
impl FromStr for LitRepr {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> { Str::from_str(s).map(Self::Str) }
}
...
impl Default for NumRepr {
    fn default() -> Self { Self::Integer(usize::default()) }
}
